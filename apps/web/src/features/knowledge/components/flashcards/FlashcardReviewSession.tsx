...
export const FlashcardReviewSession: React.FC<FlashcardReviewSessionProps> = ({ onClose }) => {
  ...
  useEffect(() => {
    if (currentCard) {
      ...
      if (currentCard.vocabulary?.audioUrl) {
        setTimeout(() => {
          playAudio();
        }, 300);
      }
          ...
          <div className="absolute inset-0 backface-hidden bg-[#ffffff]/[0.015] backdrop-blur-xl rounded-[24px] p-8 border border-white/5 rotate-y-180 overflow-y-auto custom-scrollbar flex flex-col justify-between overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <div className="space-y-4">
                ...
                {/* Dictionary definitions if present */}
                {currentCard.vocabulary?.meanings && (currentCard.vocabulary.meanings as any[]).length > 0 && (
                  <div className="mt-4">
                    ...
                    <div className="space-y-2 text-sm font-light text-gray-300">
                      {(currentCard.vocabulary.meanings as any[]).slice(0, 2).map((m: any, idx) => (
                ...
                {/* Synonyms */}
                {currentCard.vocabulary?.meanings && (currentCard.vocabulary.meanings as any[]).length > 0 &&
                  renderList(t('knowledge.review.synonyms'), (currentCard.vocabulary.meanings as any[]).find(m => m.synonyms?.length > 0)?.synonyms || [])
                }
              </div>
            </div>
            
            <div className="text-center text-[10px] font-mono text-gray-600 uppercase tracking-widest mt-6 animate-pulse border-t border-white/5 pt-4 relative z-10">
