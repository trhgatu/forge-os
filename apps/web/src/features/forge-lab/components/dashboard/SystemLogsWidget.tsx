...
export const SystemLogsWidget: React.FC<SystemLogsWidgetProps> = ({ projects, isLoading }) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between px-1">
        <Label variant="dim" className="text-xs font-bold uppercase tracking-widest block">System Logs</Label>
      </div>
      ...
      <div className="relative pl-6 space-y-8 before:absolute before:top-3 before:left-[11px] before:w-[2px] before:h-full before:bg-linear-to-b before:from-white/10 before:to-transparent">
        {isLoading ? (
          Array.from({ length: 4 }).map((_, index) => (
            <div key={index} className="relative group space-y-2">
              <div className="absolute -left-[23px] top-0 w-6 h-6 rounded-full bg-[#0c0c0e] flex items-center justify-center border border-white/10 z-10">
        ...
        {!isLoading && aggregatedLogs.length === 0 && (
