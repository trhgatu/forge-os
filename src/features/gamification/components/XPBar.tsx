...
export const XPBar: React.FC<XPBarProps> = ({ compact = false }) => {
    ...
    return (
        <div className={cn(
            ...
            compact ? "justify-center gap-0" : "p-3 bg-white/5 rounded-sm border border-white/5 hover:border-forge-cyan/30 shadow-lg group-hover:shadow-[0_0_15px_rgba(34,211,238,0.1)]"
        )}>
            ...
            <div className="relative group cursor-pointer">
                ...
                <div className={cn(
                    ...
                    <div className="flex items-center justify-center w-6 h-6 bg-zinc-900 rounded-sm border border-zinc-700 text-forge-cyan shadow-[0_0_8px_rgba(34,211,238,0.3)]">
                        <Sparkles size={14} className="text-forge-cyan drop-shadow-[0_0_3px_rgba(34,211,238,0.8)]" />
                    </div>

                    {!compact && (
                        <div className="flex flex-col leading-none">
                            <span className="text-[9px] text-forge-cyan/80 font-[family-name:var(--font-rajdhani)] uppercase tracking-widest mb-0.5">{t('gamification.level')}</span>
                            <span className="text-xl font-bold text-white font-[family-name:var(--font-rajdhani)] tracking-wide drop-shadow-md">{String(stats.level).padStart(2, '0')}</span>
                        </div>
                    )}
                </div>
                ...
                <div className="absolute bottom-full left-0 mb-4 w-72 p-0 rounded-sm bg-[#050505] border border-forge-cyan/30 shadow-[0_0_30px_rgba(0,0,0,0.8)] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 overflow-hidden backdrop-blur-xl">
                    ...
                    <div className="bg-zinc-900/90 px-4 py-2 border-b border-white/10 flex justify-between items-center relative overflow-hidden">
                        ...
                        <span className="text-sm font-[family-name:var(--font-rajdhani)] text-forge-cyan uppercase tracking-[0.2em] relative z-10 font-bold">{t('gamification.operator_stats')}</span>
                    ...
                    <div className="p-4 space-y-5 relative">
                        ...
                        <div className="relative flex items-center justify-between border-b border-white/5 pb-3">
                            <span className="text-[10px] text-gray-500 uppercase tracking-wider font-[family-name:var(--font-rajdhani)]">{t('gamification.rank_designation')}</span>
                        ...
                        <div className="space-y-2 relative">
                            <div className="flex justify-between items-end">
                                <span className="text-[10px] text-gray-500 uppercase tracking-widest font-[family-name:var(--font-rajdhani)]">{t('gamification.experience_log')}</span>
                                <span className="text-sm font-[family-name:var(--font-rajdhani)] text-forge-cyan font-bold">{Math.floor(stats.xp)} <span className="text-gray-600 font-normal">/</span> {nextLevelXp} XP</span>
                            </div>
                        ...
                        <div className="pt-2 flex justify-between items-center relative">
                            <div className="flex items-center gap-2 text-xs text-amber-500 font-mono bg-amber-500/10 px-2 py-1 rounded-sm border border-amber-500/20">
                                ...
                                <span className="font-bold font-[family-name:var(--font-rajdhani)] tracking-wide">{stats.streak} {t('gamification.day_streak').toUpperCase()}</span>
                            </div>
                            <span className="text-[9px] text-zinc-600 font-mono tracking-tighter">ID: {user?.id ? `${user.id.substring(0, 4)}...${user.id.substring(user.id.length - 4)}` : 'UNK'}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {!compact && (
                <div className="flex flex-col gap-1.5 flex-1 group/bar">
                    <div className="flex justify-between items-end px-0.5">
                        <span className="text-[9px] uppercase tracking-[0.2em] text-zinc-500 font-medium group-hover/bar:text-forge-cyan transition-colors duration-300">{t('gamification.progression')}</span>
                        <span className="text-[10px] font-mono text-zinc-400 group-hover/bar:text-white transition-colors duration-300">{Math.round(progress)}%</span>
                    </div>
