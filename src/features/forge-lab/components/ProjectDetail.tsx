...
import { useProject, Tab } from "../hooks/useProject";
...
export const ProjectDetail: React.FC<ProjectDetailProps> = ({
    ...
    return (
      <div className="max-w-7xl mx-auto p-6 md:p-10 pb-32">
        ...
        <GlassCard className="p-12 text-center">
          ...
          <p className="text-gray-400 mb-6">{error ? String(error) : "Project not found"}</p>
          <button
            onClick={onBack}
  ...
  const handleSaveResource = async () => {
    try {
      ...
      await updateLinks(updatedLinks); // Auto toast specific "Resource updated" managed inside hook?
      // Hook toast says "Links updated". Overriding might be better?
      // No, hook handles success. We just close modal.
  ...
  const handleSync = async () => {
    try {
      // Hook handles loading state via isSyncing
      await sync();
    } catch (error) {
            ...
            <GlassCard>
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-bold text-white flex items-center gap-2">
                  <ListTodo className="text-forge-cyan" /> Task Board
                </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between pb-2 border-b border-white/5">
                      ...
                      <span className="text-xs font-mono bg-white/5 px-2 py-0.5 rounded text-gray-500">
                    ...
                    {project.taskBoard.todo.map((task) => (
                      <div
                        key={task.id}
                        className="p-3 rounded-xl bg-white/5 border border-white/5 hover:border-white/10 transition-all cursor-move group"
                      >
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between pb-2 border-b border-white/5">
                      ...
                      <span className="text-xs font-mono bg-forge-cyan/10 px-2 py-0.5 rounded text-forge-cyan">
                    ...
                    {project.taskBoard.inProgress.map((task) => (
                      <div
                        key={task.id}
                        className="p-3 rounded-xl bg-forge-cyan/5 border border-forge-cyan/20 hover:border-forge-cyan/30 transition-all cursor-move"
                      >
                        <div className="flex justify-between items-start mb-2">
                          <span
                            className={cn(
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between pb-2 border-b border-white/5">
                      ...
                      <span className="text-xs font-mono bg-emerald-500/10 px-2 py-0.5 rounded text-emerald-400">
                    ...
                    {project.taskBoard.done.map((task) => (
                      <div
                        key={task.id}
                        className="p-3 rounded-xl bg-white/5 border border-white/5 opacity-60 hover:opacity-100 transition-all"
                      >
                        <div className="flex justify-between items-start mb-2">
        ...
        {activeTab === "resources" && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {project.links?.map((link, index) => (
              <GlassCard key={index} className="relative group">
                ...
                <div className="flex items-start gap-4 mb-4">
                  <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center shrink-0">
                    {link.icon === "github" ? <Github size={20} /> : <LinkIcon size={20} />}
                  </div>
                  <div>
                    <h3 className="font-bold text-white mb-1 line-clamp-1">{link.title}</h3>
                    <p className="text-xs text-gray-500 line-clamp-1">{link.url}</p>
                  </div>
                </div>
                <a
                  href={link.url}
                  target="_blank"
                  rel="noreferrer"
                  className="block w-full py-2 rounded-lg bg-white/5 text-center text-xs font-bold text-gray-400 hover:bg-white/10 hover:text-white transition-colors"
                >
                  Open Link
                </a>
              </GlassCard>
        ...
        {activeTab === "logs" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 text-gray-300">
            {project.logs?.map((log, index) => (
              ...
              >
                <div className="absolute left-[-5px] top-0 w-2.5 h-2.5 rounded-full bg-forge-cyan shadow-[0_0_8px_rgba(6,182,212,0.8)]" />
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-forge-cyan/50"
